...
//        builder = leakySetup();
    ...
    private static JettyFactoryTreeBuilder leakySetup() {
        JettyFactoryTreeBuilder builder = new JettyFactoryTreeBuilder((jetty, ctx) -> {
            jetty.withResource(ctx.get(SimplifiedMicroserviceResourceFactory.class))
                    .withResource(ctx.get(SomeOtherResourceFactory.class))
                    .withHost("localhost")
                    .withPort(PORT);
        });
    ...
    private static JettyFactoryTreeBuilder correctSetup() {
        JettyFactoryTreeBuilder builder = new JettyFactoryTreeBuilder((jetty, ctx) -> {
            jetty.withJersey(rb -> rb.withResource(ctx.get(SimplifiedMicroserviceResourceFactory.class)).withPathSpec("/servlet1/*"), new FactoryTemplateName("servlet1"))
                    .withJersey(rb -> rb.withResource(ctx.get(SomeOtherResourceFactory.class)).withPathSpec("/servlet2/*"), new FactoryTemplateName("servlet2"))
                    .withResource(ctx.get(SomeOtherResourceFactory.class))
                    .withHost("localhost")
                    .withPort(PORT);
        });
