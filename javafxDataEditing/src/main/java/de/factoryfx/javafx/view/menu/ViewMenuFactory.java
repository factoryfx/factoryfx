...
public class ViewMenuFactory<V> extends FactoryBase<Menu,V>{
    ...
    @Override
    public LiveCycleController<Menu, V> createLifecycleController() {
        return () -> createMenu();
    }
    ...
    private Menu createMenu() {
        ...
        for (final ViewMenuItemFactory<V> item: items.get()) {
            menu.getItems().add(item.instance());
        }
        return menu;
    }
