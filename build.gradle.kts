...
// the nmcp plugin registers its tasks without a task group, which IDEs display as a blank group
fun TaskContainer.groupNmcpTasks() = configureEach {
    if (group.isNullOrEmpty() && (name.startsWith("nmcp") || name == "cleanupDirectory")) {
        group = "publishing"
    }
}

tasks.groupNmcpTasks()
